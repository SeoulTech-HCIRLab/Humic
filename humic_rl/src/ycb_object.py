...
class YCBObject(object):
    def __init__(self, model):
        ...
        self.modelPath = self.modelPath.replace('humic_rl/src', 'humic_gazebo/models/{0}/{1}.sdf'.format(self.modelName, self.modelName[4:]))
    ...
    def setPose(self, position_check=False, delete=False):
        ...
        # return np.array([self.object_position.position.x, self.object_position.position.y, self.object_position.position.z, 
        #                     self.object_position.orientation.x, self.object_position.orientation.y, self.object_position.orientation.z])
